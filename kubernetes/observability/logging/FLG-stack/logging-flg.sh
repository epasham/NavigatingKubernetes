...
helm upgrade --install  loki grafana/loki -n observability --create-namespace -f ../ALG-stack/loki-values.yaml


print_header "installing Grafana dashboard"
helm upgrade --install grafana grafana/grafana -n observability --create-namespace -f ../ALG-stack/grafana-values.yaml --wait
